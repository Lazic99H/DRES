...
function TransferCard() {
  ...
  const handleSubmitDeposit = async (event) => {
    if (sessionStorage.getItem('mail') !== mail && sessionStorage.getItem('verification') === 'true' && (amount > 0 || amount < 100000)){
        ...
        const res = await axios.post('http://localhost:5002/bank/transfer', {mail: mail, amount: amount, currency: currency, balance_id: balance_id});
        ...
        alert('EMAIL MATCHES YOUR EMAIL, PLEASE ENTER RECIPIENT EMAIL!')
    }
  };
    ...
    <Card className={classes.root}>
      <CardContent className={classes.content}>
        <TextField
          ...
          helperText={`Email of the user you want to transfer money to`}
          margin='normal'
          variant='outlined'
          type='email'
          value={mail}
        ...
        <TextField
          label='Amount'
          id='outlined-basic'
          helperText={`Amount of money you want to transfer`}
          margin='normal'
          variant='outlined'
          type='number'
          required
